...
const MoreInfo = ({ modalDetails }: MoreInfoProps) => (
  <div className="grid grid-cols-1 gap-5">
    <div className="flex flex-col gap-5">
      ...
      {modalDetails.mediaType === "tv" &&
        modalDetails.moreInfo &&
        modalDetails.moreInfo.networks.length > 0 && (
          <ModalContentSection title="Série diffusée sur">
            <div className="grid grid-rows-1 grid-cols-3 md:grid-cols-5 gap-2 md:gap-5">
              {modalDetails.moreInfo.networks.map(
      ...
      {modalDetails.moreInfo &&
        modalDetails.moreInfo.productionCompanies?.length > 0 && (
          <ModalContentSection title="Produit par">
            <div className="grid grid-rows-1 grid-cols-3 md:grid-cols-5 gap-2 md:gap-5">
              {modalDetails.moreInfo.productionCompanies.map(
                (company: ProductionCompany) => (
                  <CompanyPanel
                    key={company.id}
                    logo={company.logo_path}
